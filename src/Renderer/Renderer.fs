...
let fileMenu (dispatch) =
    ...
    let makeTestItem (name:string) (accelNumber:int)  =
        ...
        makeDebugItem name (Some $"CmdOrCtrl+{accelNumber+1}") (fun _ ->
            dispatch (MenuAction( MenuDrawBlockTest( TestDrawBlockT2.testMenuFunc, accelNumber), dispatch)))
    makeMenu false "File" [
        ...
        makeMenuGen (debugLevel > 0) false "Tick3 Tests" (
            TestDrawBlockT2.testsToRunFromSheetMenu // make a submenu from this list
