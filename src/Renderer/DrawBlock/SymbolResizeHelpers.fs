...
let rotatePortInfo (rotation: Rotation) (portMaps: PortMaps) : PortMaps =
    ...
    let newPortOrientation =
        ...
        |> Map.map (fun id side -> rotateSide rotation side)

    let rotatePortList currPortOrder side =
        currPortOrder
        |> Map.add (rotateSide rotation side) portMaps.Order[side]

    let newPortOrder =
        (Map.empty, [ Top; Left; Bottom; Right ])
        ||> List.fold rotatePortList
    { Orientation = newPortOrientation; Order = newPortOrder }

let adjustPosForRotation (rotation: Rotation) (h: float) (w: float) (pos: XYPos) : XYPos =
    let posOffset =
        match rotation with
        | Degree90
...
let rotateSymbol (rotation: Rotation) (sym: Symbol) : Symbol =
    ...
    | Custom _ ->
        ...
        let getHW (sym: Symbol) =
            { X = sym.Component.W; Y = sym.Component.H }
        let sym' = { sym with PortMaps = portMaps } |> autoScaleHAndW
    ...
    | _ ->
        ...
        let newSTransform =
            ...
            | true ->
                { sym.STransform with
                    Rotation = combineRotation (invertRotation rotation) sym.STransform.Rotation } // hack for rotating when flipped
            | false ->
                { sym.STransform with
...
let rec rotateAntiClockByAng (rotAngle: Rotation) (sym: Symbol) : Symbol =
    ...
    | deg ->
        ...
        match deg with
        | Degree90 -> newSym
        | Degree180 -> rotateAntiClockByAng (Degree90) newSym
        | Degree270 -> rotateAntiClockByAng (Degree180) newSym
...
let flipSideHorizontal (edge: Edge) : Edge =
    match edge with
    | Left
    | Right -> edge |> rotateSide Degree180
    | _ -> edge
...
let flipSymbol (orientation: FlipType) (sym: Symbol) : Symbol =
    ...
    |> (fun sym ->
        ...
        | FlipHorizontal -> sym
        | FlipVertical -> sym |> rotateSymbol Degree180)
...
let showCompCorners (model: Model) (cornerShow) (compIds: ComponentId list) =
    ...
    let updateSymbolInMap prevMap cId =
        prevMap
        |> if (Map.containsKey cId model.Symbols) then
...
let manualSymbolResize
    ...
    let scaledSymBox, symBox =
        outerProduct compBox scale |> swapXYByRot, compBox |> swapXYByRot

    /// Vector outer product
    let outerProduct (a: XYPos) (b: XYPos) = { X = a.X * b.X; Y = a.Y * b.Y }

    /// apply function f to both the components of xy
    let xyApplyF (f: float -> float) (xy: XYPos) = { X = f xy.X; Y = f xy.Y }
